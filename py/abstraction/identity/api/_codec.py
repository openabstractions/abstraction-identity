...
class ProofRequirement:
    ...
    user: Proof | None = None
    process: Proof | None = None
    path: Proof | None = None
    package: Proof | None = None
    code: Proof | None = None
...
class ProofFailure:
    ...
    attribute: str = ""
    have: Proof | None = None
    need: Proof | None = None
    why: str = ""


def _write_proof_requirement(out, v, depth):
    _closed_enum(Proof, v.user)
    _closed_enum(Proof, v.process)
    _closed_enum(Proof, v.path)
    _closed_enum(Proof, v.package)
    _closed_enum(Proof, v.code)
    out += b"{"
    out += b"\n"
    _pad(out, depth + 1)
    _esc(out, "user")
...
def _write_proof_failure(out, v, depth):
    _closed_enum(Proof, v.have)
    _closed_enum(Proof, v.need)
    out += b"{"
    out += b"\n"
    _pad(out, depth + 1)
    _esc(out, "attribute")
    out += b": "
    _esc(out, v.attribute)
    out += b","
    out += b"\n"
    _pad(out, depth + 1)
    _esc(out, "have")
...
def encode(v):
    out = bytearray()
    _write_proof_requirement(out, v, 0)
    out += b"\n"
    return bytes(out)
